...
// Структура для хранения результата экзамена
struct ExamResult {
    string subject;
    int score;
    ...
    // Добавляем конструктор копирования для ExamResult
    ExamResult(const ExamResult& other) : subject(other.subject), score(other.score) {}
    
    // Добавляем оператор присваивания для ExamResult
    ...
    // Добавляем перемещающий конструктор и оператор присваивания
...
struct Student {
    ...
    // Явно объявляем конструкторы и операторы присваивания
    ...
    Student(const Student& other) : 
        ...
        exam_results(other.exam_results) {} // Используем конструктор копирования DynamicArray
    ...
    Student& operator=(const Student& other) {
        if (this != &other) {
            ...
            age = other.age;
            school_id = other.school_id;
            exam_results = other.exam_results; // Используем оператор присваивания DynamicArray
        }
        return *this;
    }
    ...
    // Проверяет, сдал ли хотя бы один экзамен на 100 баллов
...
// Функция для обработки данных одним потоком
...
int main() {
    ...
    // Создание потоков и локальных статистик
    ...
    // Ожидание завершения потоков и объединение результатов
    Map<int, SchoolStats> multi_result;
    
    for (int i = 0; i < num_threads; ++i) {
        threads[i].join();
        
        // Объединение результатов из локальной статистики
